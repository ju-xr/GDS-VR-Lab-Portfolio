...
//Please see https://forum.unity.com/threads/oculus-handtracking-with-custom-hand-model.810678/
public class FingerIK : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (RHS == null)
            RHS = GameObject.FindWithTag("RightOVRHand").GetComponent<OVRSkeleton>();

        if (LHS == null)
            LHS = GameObject.FindWithTag("LeftOVRHand").GetComponent<OVRSkeleton>();
    }
    ...
    private void UpdateHand(HandTrackingFingerMap h, OVRSkeleton s) {
        ...
        SetProperties(h.Hand_Thumb1, s.Bones[(int)bid.Hand_Thumb0].Transform);
